...
public class ZombieHorseFarmTileentity extends VillagerTileentity implements ITickableBlockEntity {

    // Update the loot table for pigs instead of iron golems
    private static ResourceKey<LootTable> ZOMBHORSE_LOOT_TABLE = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.withDefaultNamespace("entities/zombie_horse"));
    ...
    public ZombieHorseFarmTileentity(BlockPos pos, BlockState state) {
        super(ModTileEntities.ZOMBIE_HORSE_FARM.get(), ModBlocks.ZOMBIE_HORSE_FARM.get().defaultBlockState(), pos, state);
...
//            }
        } else if (timer >= getZombHorseKillTime()) {
...
//            // VillagerBlockBase.playVillagerSound(level, getBlockPos(), SoundEvents.CREEPER_DEATH);
    ...
    private List<ItemStack> getDrops() {
        ...
        LootParams.Builder builder = new LootParams.Builder(serverWorld)
                .withParameter(LootContextParams.THIS_ENTITY, new ZombieHorse(EntityType.ZOMBIE_HORSE, level)) // Change to Pig
        ...
        LootTable lootTable = serverWorld.getServer().reloadableRegistries().getLootTable(ZOMBHORSE_LOOT_TABLE);
    ...
    public static int getZombHorseTimeTime() {
        return Main.SERVER_CONFIG.zombieHorseSpawnTime.get() - 20 * 10;
    }

    public static int getZombHorseKillTime() {
        return getZombHorseTimeTime() + 20 * 10;
    }
